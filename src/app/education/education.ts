...
@Component({ selector: 'app-education', standalone: false, templateUrl: './education.html' })
export class Education implements OnInit {
  ...
  ngOnInit(): void {
    ...
    ).subscribe(data => { this.educationList = data; this.cdr.detectChanges(); });
  }

  agregarNueva() {
    const nueva = { degree: 'Nuevo Título', university: 'Universidad', startDate: '', endDate: '' };
    this.educationService.create(nueva);
  }
  ...
  eliminar(id: string) {
    if(confirm('¿Eliminar educación?')) this.educationService.delete(id);
  }
}